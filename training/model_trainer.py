...
class ModelTrainer:
    ...
    def train_model(self):
        ...
        for i_episode in range(self.episodes):
            print(f'starting game {i_episode}, epsilon rate = {self.epsilon_threshold}')
            pong = Pong(self.agent, end_score=1)
            ...
            while not done:
                ...
                self.optimize_model()
                epsilon_threshold = self.epsilon_threshold * EPSILON_DECAY if self.epsilon_threshold > EPSILON_END else EPSILON_END
                self.agent.epsilon_threshold = epsilon_threshold
